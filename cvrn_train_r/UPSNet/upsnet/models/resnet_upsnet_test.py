...
class resnet_upsnet_test(resnet_rcnn):

    def __init__(self, backbone_depth):
        super(resnet_upsnet_test, self).__init__()
    ...
    def forward(self, data, label=None):
        ...
        if label is not None:
            ...
            if config.network.has_fcn_head:
                if label is not None and config.train.fcn_with_roi_lcls_idxoss:
            ...
            if config.network.has_rpn:
                output['rpn_cls_loss'] = rpn_cls_loss.unsqueeze(0)
                output['rpn_bbox_loss'] = rpn_bbox_loss.unsqueeze(0)
                output['rpn_cls_prob_src'] = rpn_cls_prob
            if config.network.has_rcnn:
                output['cls_loss'] = cls_loss.unsqueeze(0)
                output['bbox_loss'] = bbox_loss.unsqueeze(0)
                output['rcnn_accuracy'] = rcnn_acc.unsqueeze(0)
            if config.network.has_mask_head:
                output['mask_loss'] = mask_loss.unsqueeze(0)
            if config.network.has_fcn_head:
                output['fcn_loss'] = fcn_loss.unsqueeze(0)
            if config.network.has_panoptic_head:
                output['panoptic_loss'] = panoptic_loss.unsqueeze(0)
                output['panoptic_accuracy'] = panoptic_acc.unsqueeze(0)
            ...
            # if config.network.has_rpn:
                # rpn_cls_prob_softmax = []
                # for cls_score in rpn_cls_score:
                #     cls_score = F.softmax(cls_score, dim=1)
                #     rpn_cls_prob_softmax.append(cls_score)
            ...
            if config.network.has_mask_head:
                ...
                # mask_prob_softmax = F.softmax(mask_score, dim=1)
            ...
            results = dict()
            # if config.network.has_rpn:
            #     results['rpn_cls_prob'] = rpn_cls_prob
            #     results['rpn_bbox_pred'] = rpn_bbox_pred
            if config.network.has_rcnn:
                results['cls_probs'] = cls_prob_all
                results['pred_boxes'] = mask_rois
                results['cls_inds'] = cls_idx
            if config.network.has_mask_head:
                results['mask_probs'] = mask_prob
                # results['mask_rois'] = mask_rois

            if config.network.has_fcn_head:
                fcn_output = self.fcn_head(*[fpn_p2, fpn_p3, fpn_p4, fpn_p5])
                fcn_prob = torch.max(fcn_output['fcn_output'], dim=1)[1]

            if config.network.has_fcn_head:
                results['fcn_prob'] = fcn_prob
            ...
            if config.network.has_panoptic_head:
                results['panoptic_outputs'] = panoptic_output
                results['panoptic_cls_inds'] = cls_idx
                results['panoptic_cls_probs'] = cls_prob
            return results
...
def resnet_101_upsnet_test():
    return resnet_upsnet_test([3, 4, 23, 3])

def resnet_50_upsnet_test():
    return resnet_upsnet_test([3, 4, 6, 3])
