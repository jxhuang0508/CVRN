...
class Vistas_7cls(BaseDataset):

    def __init__(self, image_sets, flip=False, proposal_files=None, phase='train', result_path=''):

        super(Vistas_7cls, self).__init__()

        self.image_dirs = {
            'train': os.path.join(config.dataset.dataset_path, 'images', 'train'),
            'val': os.path.join(config.dataset.dataset_path, 'images', 'val'),
    ...
    def vis_all_mask(self, all_boxes, all_masks, save_path=None):
        ...
        for i in range(len(self.roidb)):
            ...
            if save_path is None:
                ...
                fig.savefig(os.path.join(save_path, '{}'.format(self.roidb[i]['image'].split('/')[-1])), dpi=200)
            plt.close('all')
    ...
    # evaluate results only using AP50
    ...
    def evaluate_ssegs_folder(self, res_file_folder):
        ...
        confusion_matrix = np.zeros((config.dataset.num_seg_classes, config.dataset.num_seg_classes))
        for i, roidb in enumerate(self.roidb):
            seg_gt = np.array(Image.open(roidb['image'].replace('images', 'labels_16cls').replace('.jpg', '_labelTrainIds.png'))).astype('float32')
            # seg_gt_img = Image.open(roidb['image'].replace('images', 'labels_16cls').replace('.jpg', '_labelTrainIds.png'))
            # seg_gt = np.array(seg_gt_img.copy()).astype('float32')
            # seg_gt_img.close()

            seg_pathes = os.path.split(roidb['image'].replace('images', 'labels_16cls').replace('.jpg', '_labelTrainIds.png'))
            # res_image_name = seg_pathes[-1].replace('_labelTrainIds.png','.png')
            res_image_name = seg_pathes[-1].replace('_labelTrainIds.png','_color.png')
            res_save_path = os.path.join(res_file_folder, res_image_name)

            seg_pred = Image.open(res_save_path)
            # seg_pred_img = Image.open(res_save_path)
            # seg_pred = np.array(seg_pred_img.copy())
            # seg_pred_img.close()
    ...
    def ssegs_folder(self, res_file_folder):
        all_ssegs = []
        for i, roidb in enumerate(self.roidb):
            seg_pathes = os.path.split(roidb['image'].replace('images', 'labels_16cls').replace('.jpg', '_labelTrainIds.png'))
            # res_image_name = seg_pathes[-1].replace('_labelTrainIds.png','.png')
