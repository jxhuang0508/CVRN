...
class Synthia_7cls(BaseDataset):

    def __init__(self, image_sets, flip=False, proposal_files=None, phase='train', result_path=''):

        super(Synthia_7cls, self).__init__()

        self.image_dirs = {
            'train': os.path.join(config.source_dataset.dataset_path, 'images'),
        ...
        self.panoptic_json_file = os.path.join(config.source_dataset.dataset_path, 'annotations', 'synthia_fine_val.json')
        # self.panoptic_gt_folder = os.path.join(config.dataset.dataset_path, 'panoptic')
        ...
        if image_sets[0] == 'demoVideo':
            ...
            assert phase == 'test'
            im_path = [_.strip() for _ in open('data/synthia/split/demoVideo_img.txt', 'r').readlines()]
        ...
        if phase == 'train' and len(image_sets) > 1:
            ...
            for image_set, proposal_file in zip(image_sets, proposal_files):
                dataset = JsonDataset('synthia_' + image_set,
                                      image_dir=self.image_dirs[image_set],
                                      anno_file=os.path.join(config.source_dataset.dataset_path, 'annotations_7cls_filtered', self.anno_files[image_set]))
            ...
            assert len(image_sets) == 1
            self.dataset = JsonDataset('synthia_' + image_sets[0],
                                       image_dir=self.image_dirs[image_sets[0]],
                                       anno_file=os.path.join(config.source_dataset.dataset_path, 'annotations_7cls_filtered',
    ...
    def __getitem__(self, index):
        ...
        if config.network.has_fcn_head:
            if self.phase != 'test':
                # seg_gt = np.array(Image.open(self.roidb[index]['image'].replace('images', 'labels').replace('.png', '_labelTrainIds.png')))
                seg_gt = np.array(Image.open(self.roidb[index]['image'].replace('images', 'labels_16cls').replace('.png', '_labelTrainIds.png')))
                ...
                if config.network.has_mask_head:
                    ...
                    for i in range(len(label['gt_classes'])):
                        img = Image.new('L', (round(im_blob.shape[-1] / im_scales[0]), round(im_blob.shape[-2] / im_scales[0])), 0)
        ...
        # return data.copy(), label.copy(), index
    ...
    def evaluate_ssegs(self, pred_segmentations, res_file_folder):
        ...
        confusion_matrix = np.zeros((config.source_dataset.num_seg_classes, config.source_dataset.num_seg_classes))
        for i, roidb in enumerate(self.roidb):
            ...
            confusion_matrix += self.get_confusion_matrix(seg_gt, seg_pred, config.source_dataset.num_seg_classes)
