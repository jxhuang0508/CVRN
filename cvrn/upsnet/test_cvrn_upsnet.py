...
def upsnet_test():
    ...
    if args.eval_only:
        ...
        test_dataset.evaluate_panoptic_folder(os.path.join(final_output_path, 'results', 'pans_unified'))
        sys.exit()
    ...
    if config.test.vis_mask:
        ...
        all_panos = test_dataset.get_unified_pan_result(results['all_ssegs'], results['all_panos'], results['all_pano_cls_inds'], stuff_area_limit=config.test.panoptic_stuff_area_limit)
        test_dataset.evaluate_panoptic(all_panos, os.path.join(final_output_path, 'results', 'pans_unified'))
